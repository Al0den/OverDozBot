...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!weekSubmitInteraction) {
            return interaction.followUp('Temps écoulé. La configuration a été annulée.');
        }

        await weekSubmitInteraction.deferReply();
        ...
        while (!finished) {
            ...
            if (buttonInteraction.customId === 'add_item') {
                ...
                await addItemSubmitInteraction.deferReply();
                ...
                await addItemSubmitInteraction.followUp(`Article ajouté : ${item} (${amount})`);
            }
        ...
        try {
            ...
            if (qdfData[`week-${week}`]) {
                await weekSubmitInteraction.followUp({
                    content: `La semaine ${week} est déjà configurée. Veuillez choisir une autre semaine.`,
                });
                return;
            }
